...
Describe "Connect-DfContext" {
    Context "valid project folder" {
        BeforeAll {
            Mock Get-DfProject { return @{ Environment = @{ SubscriptionId = "TheSubscription" }} } -ModuleName DeploymentFramework
            Mock Connect-AzAccount {} -ModuleName DeploymentFramework
        }
