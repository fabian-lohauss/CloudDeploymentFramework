...
function New-CdfServiceTemplate {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Name
    ...
    $ServiceTemplateFolder = Join-Path (Get-CdfProject).ServicesPath -ChildPath $Name -AdditionalChildPath "v1.0"
    ...
    $ServiceTemplate | Export-CdfServiceTemplate
