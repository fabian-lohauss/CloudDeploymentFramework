Function Connect-DfRdp {
    ...
    Write-DfLog ("Connecting to Azure with tenantId: {0}, subscriptionId: {1}, accountId: {2}" -f $TenantId, $SubscriptionId, $AccountId)

    Write-DfLog ("Checking if Azure PowerShell module is installed")
    $AzModule = Get-Module -ListAvailable -Name Az
    if ($null -eq $AzModule) {
        Write-DfLog ("Azure PowerShell module not found, installing")
    ...
    Write-DfLog ("Checking if Azure CLI is installed")
    $AzCli = Get-Command -Name az -ErrorAction SilentlyContinue
    if ($null -eq $AzCli) {
        Write-DfLog ("Azure CLI not found, installing")
    ...
    Write-DfLog ("Checking if Azure PowerShell module is up to date")
    $AzModule = Get-Module -ListAvailable -Name Az
    if ($null -eq $AzModule) {
        throw "Azure PowerShell module not found"
    }

    Write-DfLog ("Checking if Azure Az is logged in")
    $CmdletContext = Get-AzContext 
    Write-DfLog ("Current Azure context: {0}" -f $CmdletContext.Name)
    if (($CmdletContext.Tenant.Id -ne $TenantId) -or ($CmdletContext.Subscription.Id -ne $SubscriptionId) -or ($CmdletContext.Account.Id -ne $AccountId)) {
        Write-DfLog ("Expected Azure context: tenantId: {0}, subscriptionId: {1}, accountId: {2} but got tenantId: {3}, subscriptionId: {4}, accountId: {5}" -f $TenantId, $SubscriptionId, $AccountId, $CmdletContext.Tenant.Id, $CmdletContext.Subscription.Id, $CmdletContext.Account.Id)
    ...
    Write-DfLog ("Checking if Azure CLI is logged in")
    $AzContext = az account show | ConvertFrom-Json
    Write-DfLog ("Current Azure context: {0}" -f $AzContext.name)
    if (($AzContext.tenantId -ne $TenantId) -or ($AzContext.user.name -ne $AccountId)) {
        Write-DfLog ("Expected Azure context: tenantId: {0}, accountId: {1} but got tenantId: {2}, accountId: {3}" -f $TenantId, $AccountId, $AzContext.tenantId, $AzContext.user.name)
    ...
    Write-DfLog ("Checking if Azure Az is set to the correct subscription")
    if ($AzContext.id -ne $SubscriptionId) {
        Write-DfLog ("Expected Azure subscriptionId: {0} but got subscriptionId: {1}" -f $SubscriptionId, $AzContext.id)
    ...
    Write-DfLog ("Checking if VM '{0}' exists" -f $Name)
    $VM = Get-AzVM -Name $Name
    if ($null -eq $VM) {
        throw ("VM '{0}' not found" -f $Name)
    }

    Write-DfLog ("Checking if VM '{0}' has the required tags" -f $VM.Name)
    ...
    Write-DfLog ("Checking if bastion '{0}' exists in resource group '{1}'" -f $VM.Tags.DfBastionName, $VM.Tags.DfBastionResourceGroup)
    $Bastion = Get-AzBastion -Name $VM.Tags.DfBastionName -ResourceGroupName $VM.Tags.DfBastionResourceGroup
    if ($null -eq $Bastion) {
        throw "Bastion not found"
    }

    Write-DfLog ("Connecting to VM '{0}' via bastion '{1}' in resource group '{2}'" -f $VM.Name, $VM.Tags.DfBastionName, $VM.Tags.DfBastionResourceGroup)
